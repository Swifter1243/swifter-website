<script lang="ts">
    ...
    const { trackUrl, width="100%", height = "300px" } = $props<{
        trackUrl: string,
        width?: string
        height?: string
    }>()
    ...
    onMount(async () => {
        try {
            const oEmbedUrl = `https://soundcloud.com/oembed?format=json&url=https://soundcloud.com${trackUrl}`;
            const res = await fetch(oEmbedUrl);
            const data = await res.json();

            thumbnailUrl = data.thumbnail_url.replace("-large", "-t500x500");

            const rawTitle = data.title as string
            const song = rawTitle.substring(0, rawTitle.search('by'))
...
<div class=root style="width: {width}; height: {height}">
    {#if !iframeVisible}
        ...
        <div onclick={loadGizmo} style="width: 100%; height: 100%">
            {#if thumbnailUrl}
                <img class="thumbnail" src={thumbnailUrl} alt="Soundcloud thumbnail" loading="lazy"/>
            {/if}
            {#if title}
            ...
            {/if}
            <svg class="play-button" fill="#ff5500" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
    ...
    {:else} 
        <iframe
            ...
            frameborder="no"
            allow="autoplay"
            src="https://w.soundcloud.com/player/?url=https://soundcloud.com{trackUrl}&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=true&auto_play=true"
        ></iframe>
    {/if}
</div>